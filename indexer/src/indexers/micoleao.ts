/**
 * Mico Leão Dublado Indexer — Scrapes micoleaodublado.com
 */
...
const MICOLEAO_SITE_URL = process.env.MICOLEAO_SITE_URL || "https://micoleaodublado.com";
const HOST_FRAGMENT = "micoleaodublado.com";
...
async function scrapePost(postUrl: string, imdb: string | undefined, signal: AbortSignal): Promise<TorrentResult[]> {
  try {
    ...
    return magnets.map((magnet) => {
      ...
      return {
        title: `Mico Leão | ${pageTitle || "Conteúdo"}`,
        imdb,
...
export async function scrapeMicoLeao(req: IndexerRequest, signal: AbortSignal): Promise<IndexerResponse> {
  ...
  for (const query of searchQueries) {
    ...
    try {
      ...
      console.error(`[micoleao] Search failed for "${query}": ${msg}`);
    }
  }
