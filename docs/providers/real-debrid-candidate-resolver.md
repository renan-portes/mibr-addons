...
The subsequent diagnostic run confirmed `LISTENING`, a live process and the expected loopback publication but still timed out health. The HTTP boundary now emits only fixed markers for accepted requests, health response start/completion and premature client abort. The health response is synchronous, provider-independent and byte-length framed; local HTTP/1.1 and keep-alive tests pass for both `127.0.0.1` and wildcard internal bind. A further runtime run is required to establish whether the request reaches Node and whether the response is flushed before the reset.
