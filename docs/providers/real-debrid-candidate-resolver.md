...
Phase 1, the isolated offline HTTP Docker validation, is complete. The next isolated layer is a separate client-access launcher that remains offline/fake and requires literal explicit authorization. Loopback is the safe default. LAN mode requires a second literal authorization plus an explicit RFC1918 address verified against an active local interface; wildcard, public, loopback, hostname and IPv6 LAN publication are rejected. Exactly one port may be published, and health, the stable experimental manifest, and the offline stream response must pass before readiness is announced. The launcher changes no firewall, creates no WAN forwarding, reverse proxy, DNS or tunnel, and never enables Real-Debrid. Stremio, Nuvio and playback remain pending until a separately authorized runtime execution.
...
Temporary experimental client access remains explicitly authorized and offline/fake. It publishes exactly one operator-selected, currently unoccupied host port in `1024..65535`; it never auto-selects a replacement, and rejects an occupied port without stopping its process. Its optional lifetime is limited to an integer from `0` through `3600` seconds. LOOPBACK is the default, while LAN additionally requires a local active RFC1918 address and a second exact authorization. The standard bootstrap, manifest and router remain unchanged.

The separately authorized Real-Debrid client launcher was runtime-validated on base `dd7c9a6` in LOOPBACK mode: it published only `127.0.0.1:17007`, received a file-only token, reached `CLIENT_ACCESS_READY` and `REAL_DEBRID_MODE_ENABLED`, and validated health and the experimental manifest with HTTP `200`. It did not query `/stream`; its synthetic candidate was not sent to Real-Debrid. Manual interruption returned the expected code `130`, with complete removal of lab containers, network, and temporary files. No WAN exposure, proxy, firewall change, Stremio, Nuvio, or playback was involved. The next controlled experiment is one non-repeating `/stream` query using only legal authorized content, an exact allowlisted IMDb ID, and coherent magnet/info-hash input, followed by mandatory cleanup.

LAN access was then runtime-validated on base `0214d8d` through the normalized `eth0` interface and a single explicit `192.168.100.101:17007` TCP publication. Health and the experimental manifest were reachable from a browser on another device in the same LAN, and the Docker-bridge rejection policy was confirmed before publication. No `/stream` route was queried, no addon or playback was started, and manual interruption returned the expected code `130`. Cleanup removed every lab container, network, listening port, and observed temporary artifact without changing the existing Portainer-managed services. No WAN, proxy, firewall, DNS, tunnel, or public service participated. The next experiment remains one `/stream` request for one authorized allowlisted candidate with coherent magnet/info hash, no non-idempotent retry, no playback, and mandatory cleanup.
